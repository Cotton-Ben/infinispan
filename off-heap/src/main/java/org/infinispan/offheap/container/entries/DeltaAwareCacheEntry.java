...
import static org.infinispan.offheap.container.entries.DeltaAwareCacheEntry.Flags.EVICTED;
import static org.infinispan.offheap.container.entries.DeltaAwareCacheEntry.Flags.REMOVED;
import static org.infinispan.offheap.container.entries.DeltaAwareCacheEntry.Flags.CHANGED;
import static org.infinispan.offheap.container.entries.DeltaAwareCacheEntry.Flags.VALID;
import static org.infinispan.offheap.container.entries.DeltaAwareCacheEntry.Flags.CREATED;
 ...
 */
public class DeltaAwareCacheEntry implements OffHeapCacheEntry, StateChangingEntry {
   ...
   public DeltaAwareCacheEntry(Object key, DeltaAware value, OffHeapCacheEntry wrappedEntry) {
      setValid(true);
      this.key = key;
      this.value = value;
   ...
   public boolean skipLookup() {
      return isFlagSet(DeltaAwareCacheEntry.Flags.SKIP_LOOKUP);
   }
   ...
   private Metadata extractMetadata(OffHeapCacheEntry entry, Metadata provided) {
      if (provided != null) {
         return provided;
   ...
   public boolean isLoaded() {
      return isFlagSet(DeltaAwareCacheEntry.Flags.LOADED);
   }

   @Override
   public void setLoaded(boolean loaded) {
      setFlag(loaded, DeltaAwareCacheEntry.Flags.LOADED);
   }

   @Override
   public void setSkipLookup(boolean skipLookup) {
      setFlag(skipLookup, DeltaAwareCacheEntry.Flags.SKIP_LOOKUP);
   }
